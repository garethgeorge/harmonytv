...
class User {
  ...
  async load(conn = null) {
    ...
    const res = await conn.query(
      pgformat("SELECT * FROM users WHERE userid = %L", this.userid)
    );
    if (res.rows.length === 0) {
      throw new Error(
        "fatal error: user with id: " + this.userid + " not found"
      );
    }
  ...
  checkPassword(queryPassword) {
    if (!this.passwordsha256) return;
    const pwhash = hashPassword(this.userid, queryPassword);
    debug(
      `checkPassword() - compare hashes ${this.passwordsha256} === ${pwhash}`
    );
  ...
  async setPlaybackPositionForMedia(
    mediaid,
    position,
    total_duration,
    conn = null
...
exports.getByUsername = async (username, conn = null) => {
  if (!conn) conn = pool;
  const res = await conn.query(
    pgformat("SELECT userid FROM users WHERE username = %L", username)
  );

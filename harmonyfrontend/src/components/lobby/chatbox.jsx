...
export default observer(
  class ChatBox extends React.Component {
    ...
    constructor(props) {
      ...
      this.addMessage(
        <div>
          Type <span className="command">\?</span> for a list of commands.
        </div>,
        { kind: "info" }
      );
    ...
    registerCommand(command, handler, opts = null) {
      ...
      if (!arglist && opts.args) {
        arglist = opts.args.map(arg => {
          if (arg.optional) {
            return (
              <span key={arg.name} className="command-arg optional">
                {" [" + arg.name + "]"}
              </span>
            );
          }
          return (
            <span key={arg.name} className="command-arg">
              {" <" + arg.name + ">"}
            </span>
          );
        });
    ...
    flushCommand() {
      ...
      for (const line of this.commandBox) {
        lines.push(
          <span
            className={
              "chat-text " + line.kind + " " + line.data.classlist.join(" ")
            }
            key={line.key}
          >
            {line.text}
      ...
      setTimeout(() => {
        ...
        this.setState(state, () => {
          if (this.messages.current)
            this.messages.current.scrollTop =
              this.messages.current.scrollHeight + 1000;
        });
    ...
    receiveRelayMessage(relayMessage, mine = false) {
      ...
      if (message.type == "user-message") {
        this.addMessage(
          <span className={"user-message " + (mine ? "my-message" : "")}>
    ...
    render() {
      ...
      return (
        <div
          className={
            "chatbox " + (this.state.docked ? "docked " : "") + this.state.side
          }
        >
